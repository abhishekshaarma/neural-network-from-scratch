...
public:
    ...
    void backwards(VectorXd& input, VectorXd& target_output, double learning_rate)
    {
        ...
        for(int i = 0; i < layers.size(); i++)
        {
            ...
            layers[i].update_weight(input_, learning_rate);
               ...
               double learning_rate, int epochs)
    {
        ...
        for (int epoch = 0; epoch < epochs; ++epoch)
        {
            ...
            for (size_t i = 0; i < inputs.size(); ++i)
            {
                ...
                backwards(input, target_output, learning_rate);
            }
            epoch_loss /= inputs.size();
            std::cout << "Initial weights norm: " << layers[0].get_weights().norm() << std::endl;
